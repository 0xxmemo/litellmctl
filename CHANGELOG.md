...
- **Gateway is now a true pass-through on the hot path.** Three layered changes cut out work the Bun `/v1/*` handler used to run on every request:

  1. **Request body streaming.** The handler no longer does `await req.arrayBuffer()` unconditionally. It checks up front whether the body actually needs inspection — i.e. the caller has a per-model override configured, or a JSON→multipart audio conversion is required (`Content-Type` not already `multipart/*` on `/audio/transcriptions` / `/audio/translations`). If neither, `req.body` (a `ReadableStream`) streams straight into `fetch()` with `duplex: "half"`. Uploads now begin forwarding to LiteLLM as the client uploads, instead of waiting for the full body to be buffered at the gateway. Measurable time-to-first-byte win on long-context chat requests (100 KB+ messages), audio multipart uploads, and vision payloads — the gateway no longer sits between the client's upload and the provider's decode. Smoke-tested with a 200 KB body round-tripping byte-identical through the new streaming path.
  2. **Non-streaming usage tracking moved to response headers.** Previously every non-streaming response was cloned and the clone drained to parse `.usage` out of the JSON body. The vendored LiteLLM fork now emits `x-litellm-prompt-tokens` / `x-litellm-completion-tokens` / `x-litellm-total-tokens` directly (sourced from `response.usage` at the main non-streaming success site in `common_request_processing.py`, tolerating OpenAI `prompt_tokens`/`completion_tokens` and Anthropic `input_tokens`/`output_tokens` shapes plus dict/Pydantic access; auto-summing if only prompt+completion are supplied). The gateway reads three headers after `await fetch()` and enqueues a row — no `response.clone()`, no `.json()` parse, no async promise chain, no tee'd buffer. The entire `trackFromJson` helper is deleted.
  3. **Streaming usage tracking is now incremental.** The old `trackFromSSE` did `clone.text()` — buffering the entire streamed response into a string, splitting on newlines, scanning backwards for the usage event. O(N) memory in response length. The new path reads the clone with a `ReadableStreamReader` + rolling newline buffer, substring-filters lines so `JSON.parse` runs only on lines containing `"usage"` or `"message_start"` (skipping every content-only delta cheaply), and captures model + usage in forward order as they arrive. O(1) memory regardless of stream length; 5 000 SSE lines parsed in ~1 ms in the regression harness. UTF-8 chunk-boundary safety via `TextDecoder({ stream: true })`, verified with a test that splits a JSON line in the middle of a multi-byte sequence.

- **Dropped the per-request `/model/info` round-trip.** The gateway used to resolve `x-litellm-model-id` (a deployment UUID) back to the underlying model name by calling LiteLLM's `/model/info` endpoint on every tracked request — a 3 s-timeout fetch on the background telemetry path, doubling LiteLLM HTTP load. The vendored fork now emits the resolved model directly as `x-litellm-model-name` from inside `get_custom_headers` by reading `request_data["deployment"].litellm_params.model` (the same `Deployment` object the router already looked up for cost calculation). Tolerates both Pydantic `Deployment` and dict-shaped deployments; emits nothing when the route didn't go through the router (direct passthroughs), in which case the gateway falls through to `response.model` as before. `resolveModelById` and its fetch path are deleted.

### Fixes

- **Anthropic streaming telemetry was silently losing the resolved model name.** The old `trackFromSSE` scanned SSE events backwards and broke on the first `message_delta` with usage — which, for Anthropic's event order, appears *after* `message_start`. The `message_start` branch was effectively dead code, so `model` stayed `null` and `usage_logs.actual_model` fell back to whatever the client requested (the alias) instead of the underlying deployment. The streaming rewrite processes events in order and captures `message.model` on `message_start` before the later `message_delta`. Combined with the new `x-litellm-model-name` header, recorded `actual_model` now matches what actually ran even when the fallback chain was exercised.
- **Parser hygiene: cancel the clone reader on error.** If the streaming telemetry parser throws mid-stream (bad JSON shape, decoder panic, anything else), `reader.cancel()` now runs in the catch block. Without it, Bun's tee would retain any upstream bytes that arrived after the failure until the original response completed — a memory blip, never a correctness issue, but cheap to clean up. Confirmed via inspection that the clone is read-only: no code path can mutate bytes the client sees, no failure mode in the parser can delay or truncate the client stream.

### Operations

- **Restart both services after upgrading.** This release touches the Bun gateway (`gateway/routes/proxy.ts`) and the vendored LiteLLM submodule (`litellm/litellm/proxy/common_request_processing.py`). `litellmctl restart gateway` only cycles the Bun wrapper — run `litellmctl restart proxy` as well so the new `x-litellm-*` headers actually emit. Validated on the live install: non-streaming `/v1/chat/completions` against the `lite` tier now returns `x-litellm-prompt-tokens: 14` / `x-litellm-completion-tokens: 4` / `x-litellm-total-tokens: 18` / `x-litellm-model-name: anthropic/claude-haiku-4-5` matching `response.usage` exactly; streaming call carries `x-litellm-model-name` in the initial header frame and the stream parser picks up usage from the final `data:` event as designed.
