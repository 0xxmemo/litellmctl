...
import {
  ...
  EndpointTryCard,
  useApiKey,
} from "@/components/EndpointTryCard";
...
function getSetupCmd(baseUrl: string): string {
  return `curl -fsSL ${baseUrl}/setup/claude-code.sh | LLM_GATEWAY_API_KEY="${KEY_PLACEHOLDER}" bash`;
}
  ...
  return (
    <Card className="mb-8 border-l-4 border-l-violet-500">
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          One command sets up Claude Code to use LLM Gateway as your API
          provider.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <ul className="text-xs text-muted-foreground space-y-1 list-none pl-1">
          <li>
            • Creates <code className="text-xs">~/.claude/settings.json</code> —
            non-destructive merge
          </li>
          <li>
            • Sets <code className="text-xs">ANTHROPIC_BASE_URL</code> +{" "}
            <code className="text-xs">ANTHROPIC_AUTH_TOKEN</code>
          </li>
          <li>
            • Maps model aliases: <code className="text-xs">ultra</code> /{" "}
            <code className="text-xs">plus</code> /{" "}
            <code className="text-xs">lite</code>
          </li>
          <li>
            • Bypasses onboarding in{" "}
            <code className="text-xs">~/.claude.json</code>
          </li>
        </ul>

        <p className="text-xs text-muted-foreground">
          Works on <strong>macOS</strong> and <strong>Linux</strong>. Requires{" "}
          <code className="text-xs">claude</code> CLI and{" "}
    ...
    <div className="prose dark:prose-invert max-w-4xl mx-auto p-4 sm:p-6">
      ...
      <Card className="mb-8 border-l-4 border-l-blue-500">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Enter your key — saved in your browser's localStorage and used by
            the "Try" panels below
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          {!apiKey && (
            <p className="text-xs text-muted-foreground mt-2">
              Get your key from the <strong>API Keys</strong> tab. It's stored
